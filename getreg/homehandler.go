...
func Homehandler(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == http.MethodGet {
		...
		var sdk []Posts
		sth, errNext := conn.Query("SELECT user_id, post_id, post, username, category, STRFTIME('%H:%M', date_created) FROM activities")
		if errNext != nil {
			fmt.Println(errNext)
			return
		}
		...
		for sth.Next() {
			...
			var str Posts
			//str.Time = time.Now()
			err = sth.Scan(&str.P_Uid, &str.Pid, &str.Posted, &str.Name, &str.Category, &str.Time)
			if err != nil {
				fmt.Println(err)
				return
			}
